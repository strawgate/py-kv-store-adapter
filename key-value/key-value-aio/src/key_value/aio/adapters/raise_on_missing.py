...
class RaiseOnMissingAdapter:
    ...
    async def get(
        ...
        *,
        ...
        """
        result = await self.kv_store.get(key=key, collection=collection)
    ...
    async def get_many(
        ...
        """
        results: list[dict[str, Any] | None] = await self.kv_store.get_many(collection=collection, keys=keys)
    ...
    async def ttl(
        ...
        """
        value, ttl = await self.kv_store.ttl(key=key, collection=collection)
    ...
    async def ttl_many(
        ...
        Args:
            ...
            collection: The collection to retrieve keys from. If no collection is provided, it will use the default collection.
        """
        results: list[tuple[dict[str, Any] | None, float | None]] = await self.kv_store.ttl_many(collection=collection, keys=keys)
    ...
    async def put(self, key: str, value: dict[str, Any], *, collection: str | None = None, ttl: float | None = None) -> None:
        ...
        """
        return await self.kv_store.put(key=key, value=value, collection=collection, ttl=ttl)
        ...
        *,
        ...
        Args:
            ...
            ttl: The optional time-to-live (expiry duration) for the key-value pairs. Defaults to no TTL. Note: The
                backend store will convert the provided format to its own internal format.
        """
        return await self.kv_store.put_many(keys=keys, values=values, collection=collection, ttl=ttl)
